...
/// Internal exports used by integration tests under `tests/`. The Aomi
/// runtime itself only invokes the plugin via the C ABI generated by
/// `dyn_aomi_app!` — these symbols are not part of the dynamic-loading
/// contract. They are exposed here so we can exercise each `DynAomiTool`
/// through the SDK's `run_tool` test harness against a live backend.
#[doc(hidden)]
pub mod __test_exports {
    pub use crate::client::KairoAerodromeApp;
    pub use crate::tool::{GetGaugeSignal, GetPolicy, GetPositions, GetReceipt, ProposeAction};
}
